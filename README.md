...
服务注册时需要提供所使用的内网 base url（例如 course.hdu.edu.cn），实现接口时，传入 api 路径而不是完整 url。可以从每次请求的返回中得知请求最终实际访问的地址（例如，在 webvpn 模式下会经过 webvpn 转换）。
...
- `cookiejar` 和底层的 `http.Client` 只认 Real URL 并处理实际的网络通信
- Service 层的逻辑永远只看 Business URL
- `Transport` 充当了这两界之间的翻译官

### 自动重定向

本库依赖 `http.Client` 的默认行为自动跟随 3xx 重定向。请求完成后，中间件通过检查**最终请求的 URL**来判断是否发生了认证失效（WebVPN 掉线或 SSO 过期），而非拦截重定向响应手动解析 Location Header。
...
- `Decode`: 剥离 WebVPN 壳子，还原为原始业务地址（支持 `http-`/`https-` 前缀及任意端口后缀）
- `IsAuthFailure`: 如果最终请求 URL 的 Host 不是 `*.webvpn.hdu.edu.cn`（例如被重定向到了 `webvpn.hdu.edu.cn` 登录页），说明掉出了 WebVPN 环境
...
│                                                              │
│  ▲ (c.Next() 返回后)                                         │
│  │ 检查 resp.BusinessReqURL.Host:                            │
│  │ 如果最终请求被重定向到了 sso.hdu.edu.cn -> 触发 SSO 认证   │
...
│                                                              │
│  ▲ (c.Next() 返回后)                                         │
│  │ 检查 resp.RawResponse.Request.URL:                        │
│  │ 判断是否是 WebVPN 自身的掉线重定向 (非 .webvpn 域名):      │
│  │    -> 触发 WebVPN Reauth                                   │
│  │    -> 认证完成后 -> 重新调用本 handler 再次发起请求        │
│  │ 否则 -> 将 resp 向上返回给 serviceAuthHandler               │
└─────────────────────────────────────────────────────────────┘
     │
     ▼
┌─────────────────────────────────────────────────────────────┐
...
type Response struct {
    BusinessReqURL *url.URL       // 实际请求的业务地址（经过重定向后可能与 Request.BusinessURL 不同）
    RawResponse    *http.Response // 原始的 http.Response
    StatusCode     int
    Header         http.Header
    Body           []byte         // 响应体字节（已读取完毕，可直接使用）
}
```
...
```golang
// 当认证失效时，Do 方法会：
// 1. 检查最终请求的 URL Host 是否被重定向到了认证页面（如 sso.hdu.edu.cn）
// 2. 使用保存的 username/password 重新执行 SSO/WebVPN 认证
// 3. 重试失败的请求
